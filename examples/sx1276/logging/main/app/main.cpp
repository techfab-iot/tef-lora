...
extern "C" void app_main() {
  ...
  wifi_init_sta("TechFab_2G", "^pJEUfpz2KZ*5Qsf");
  ...
  while (ntp_result != ESP_OK) {
    ESP_LOGE("main", "Failed to update system time within 10s timeout");
    ntp_result = esp_netif_sntp_sync_wait(pdMS_TO_TICKS(10000));
  }
  ESP_LOGI(
    "main", "Time synchronized successfully, Timestamp: %" PRIu64,
