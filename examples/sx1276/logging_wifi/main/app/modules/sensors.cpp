...
float last_bl_voltage = 0;
float last_bl_current = 0;
float last_bl_active_power = 0;
float last_bl_power_factor = 0;
cJSON *telemetryCallback(cJSON *payload, uint32_t seq) {
  ESP_LOGD(pcTaskGetName(NULL), "Received telemetry: %s", cJSON_Print(payload));
  last_bl_voltage = cJSON_GetObjectItem(payload, "voltage")->valuedouble;
  last_bl_current = cJSON_GetObjectItem(payload, "current")->valuedouble;
  last_bl_active_power =
    cJSON_GetObjectItem(payload, "activePower")->valuedouble;
  last_bl_power_factor =
    cJSON_GetObjectItem(payload, "powerFactor")->valuedouble;
  return nullptr;
}
...
void monitorAcTask(void *) {
  ...
  std::string log =
    ...
    "Factor(%),\tBL:Voltage(V),\tBL:Current(A),\tBL:Active "
    "Power(W),\tBL:Power Factor(%)\tTimestamp(s),\r\n";
  while (true) {
    // VOLTAGE
    ...
    log += std::to_string(read_value) + ",\t\t";
    log += std::to_string(last_bl_voltage) + ",\t";
    log += std::to_string(last_bl_current) + ",\t";
    log += std::to_string(last_bl_active_power) + ",\t\t";
    log += std::to_string(last_bl_power_factor) + ",\t";
    if (count > 3)
    ...
    log = "log:\n";
    vTaskDelay(pdMS_TO_TICKS(kUpdateMeasurementIntervalMs));
    vTaskDelay(1);
  }
